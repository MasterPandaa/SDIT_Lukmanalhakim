...
<div class="container py-5">
    <div class="d-flex align-items-center justify-content-between mb-1">
        ...
        <span id="totalFasilitas" class="badge bg-success-subtle text-success border border-success small">Total: {{ $fasilitas->total() }}</span>
    </div>
    <p class="text-muted mb-4">Sarana dan prasarana sekolah untuk mendukung proses belajar yang nyaman dan berkualitas.</p>
    <form method="GET" action="{{ route('about.fasilitas') }}" class="card card-body mb-4 border-0 filter-card" id="fasilitasFilterForm">
        <input type="hidden" name="page" value="1">
        <div class="row g-3 align-items-end">
            <div class="col-12">
                <label class="form-label small text-uppercase fw-semibold text-muted">Cari Fasilitas</label>
                <div class="input-group input-group-sm stylish-input glow-on-focus">
                    <span class="input-group-text bg-white"><i class="fas fa-search text-muted"></i></span>
                    <input type="text" name="q" value="{{ $q ?? '' }}" class="form-control auto-submit" placeholder="Ketik nama, kategori, atau deskripsi...">
                    @if(!empty($q))
                      <a class="btn btn-outline-secondary" data-ajax="1" href="{{ route('about.fasilitas') }}" title="Bersihkan"><i class="fas fa-times"></i></a>
                    @endif
                </div>
            </div>
        </div>
    </form>
    <div id="fasilitasList">
...
<style>
  ...
  /* Ensure no shadow on cards/images */
  #fasilitasCards .card, #fasilitasCards img { box-shadow: none !important; }
  #fasilitasCards .card{transition:transform .25s ease}
  #fasilitasCards .card:hover{transform:translateY(-4px);}
  .filter-card{border-radius:.75rem}
</style>
...
<script>
  document.addEventListener('DOMContentLoaded', function(){
    (function(){
      const form = document.getElementById('fasilitasFilterForm');
      const list = document.getElementById('fasilitasList');
      const total = document.getElementById('totalFasilitas');
      if(!form || !list) return;
