        ...
        <div class="col-12">
            <div class="card border-0 shadow">
                ...
                <div class="card-body">
                    ...
                    <form method="GET" action="{{ route('admin.prestasi.index') }}" class="row g-2 align-items-center mb-3">
                        <div class="col-12 col-md-6">
                            <div class="input-group">
                                <span class="input-group-text bg-white"><i class="fas fa-search"></i></span>
                                <input type="text" name="q" value="{{ request('q') }}" class="form-control" placeholder="Cari judul, deskripsi, peraih, penyelenggara...">
                            </div>
                        </div>
                        ...
                        <div class="col-4 col-md-3 text-end">
                            ...
                            <a href="{{ route('admin.prestasi.index') }}" class="btn btn-light"><i class="fas fa-undo me-1"></i>Reset</a>
                        </div>
                    </form>
                    @include('admin.about.prestasi.partials.table', ['items' => $items])
                </div>
            </div>
        </div>
    </div>
...
<script>
  document.addEventListener('click', async function(e){
    ...
    try {
      ...
      if (typeof Swal !== 'undefined' && Swal.fire) {
        await Swal.fire({ icon: 'success', title: 'Berhasil', text: data.message || 'Prestasi berhasil dihapus' });
        window.location.reload();
      } else {
        alert(data.message || 'Prestasi berhasil dihapus');
        window.location.reload();
      }
    } catch (err) {
      if (typeof Swal !== 'undefined' && Swal.fire) {
