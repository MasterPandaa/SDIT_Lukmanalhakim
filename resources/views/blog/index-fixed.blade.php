                                ...
                                <div class="post-thumb">
                                    <a href="{{ route('blog-single', $artikel->slug) }}">
                                        @if($artikel->gambar)
                                            @php
                                                $gambarUrl = '';
                                                // Try multiple paths for image - FIXED VERSION
                                                if (file_exists(public_path('storage/artikel/' . $artikel->gambar))) {
                                                    $gambarUrl = asset('storage/artikel/' . $artikel->gambar);
                                                } elseif (\Storage::disk('public')->exists('artikel/' . $artikel->gambar)) {
                                                    $gambarUrl = \Storage::disk('public')->url('artikel/' . $artikel->gambar);
                                                } elseif (file_exists(public_path($artikel->gambar))) {
                                                    $gambarUrl = asset($artikel->gambar);
                                                } else {
                                                    $gambarUrl = asset('assets/images/blog/default.jpg');
                                                }
                                            @endphp
                                            <img src="{{ $gambarUrl }}" alt="{{ $artikel->judul }}" onerror="this.src='{{ asset('assets/images/blog/default.jpg') }}'">
                                        @else
